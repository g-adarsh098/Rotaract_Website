...
import { getFirestore, collection, addDoc, getDocs, serverTimestamp, query, orderBy, doc, getDoc, setDoc } from 'firebase/firestore';
...
const Login = ({ onLogin, onGoToAdmin }) => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // Auto-login to portal with Google credentials
      onLogin({
      ...
      <div className="w-full max-w-md bg-[#0a0a0a] border border-[#222] rounded-2xl shadow-2xl overflow-hidden relative my-8">
        ...
        <div className="p-8 sm:p-10">
          ...
          <div className="mt-6 flex items-center">
            ...
            <span className="flex-shrink-0 px-4 text-[#555] text-xs font-semibold uppercase tracking-wider">Or continue with</span>
            <div className="flex-grow border-t border-[#333]"></div>
          </div>

          <button
            type="button"
            className="w-full mt-6 py-3.5 px-4 bg-[#111] border border-[#333] text-white font-bold rounded-lg hover:bg-[#1a1a1a] hover:border-[#444] transition-all flex items-center justify-center gap-3 transform hover:-translate-y-0.5 active:translate-y-0"
      ...
      <div className="w-full max-w-md bg-[#0a0a0a] border border-[#D4AF37]/30 rounded-2xl shadow-[0_0_40px_rgba(212,175,55,0.1)] overflow-hidden relative my-8">
        ...
        <div className="p-8 sm:p-10">
          <div className="text-center mb-8">
            ...
            <p className="text-[#D4AF37] text-xs font-semibold tracking-widest uppercase letter-spacing-2">Secure Access</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Admin Passkey</label>
              <input
                type="password"
                value={passkey}
          ...
          <div className="mt-8 text-center">
            <button type="button" onClick={onBackToVoter} className="text-gray-500 text-xs font-semibold uppercase tracking-wider hover:text-white transition-colors flex items-center justify-center gap-2 mx-auto">
